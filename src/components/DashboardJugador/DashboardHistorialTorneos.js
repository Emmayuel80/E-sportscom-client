...
import getHistorialTorneos from "../../services/jugador/getHistorialTorneos";
const cantidad = 6;
// let torneoSize = 0;
const DashboardVerTorneosInscritos = () => {
  ...
  const handlePaginationChange = (event, index) => {
    getHistorialTorneos(index, cantidad, user, setTorneos, torneos);
  };

  React.useEffect(() => {
    // tournaments/:inicio/:cantidad
    if (user.token) {
      getHistorialTorneos(0, cantidad, user, setTorneos, torneos);
